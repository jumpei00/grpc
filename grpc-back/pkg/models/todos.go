...
var TodoWhere = struct {
	...
	Key:       whereHelperstring{field: "`todos`.`key`"},
...
var (
	todoAllColumns            = []string{"key", "content", "created_at", "updated_at"}
	todoColumnsWithoutDefault = []string{"key", "content", "created_at", "updated_at"}
	todoColumnsWithDefault    = []string{}
	todoPrimaryKeyColumns     = []string{"key"}
...
func FindTodo(ctx context.Context, exec boil.ContextExecutor, key string, selectCols ...string) (*Todo, error) {
	todoObj := &Todo{}
	...
	query := fmt.Sprintf(
		"select %s from `todos` where `key`=?", sel,
	)
...
func (o *Todo) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM `todos` WHERE `key`=?"
...
func (o *Todo) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindTodo(ctx, exec, o.Key)
	if err != nil {
		return err
	}
...
func TodoExists(ctx context.Context, exec boil.ContextExecutor, key string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from `todos` where `key`=? limit 1)"
...
func (o *Todo) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return TodoExists(ctx, exec, o.Key)
}
